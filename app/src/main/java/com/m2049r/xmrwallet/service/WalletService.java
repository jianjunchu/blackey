...
public class WalletService extends Service {
    ...
    private class MyWalletListener implements WalletListener {
        ...
        public void moneySpent(String txId, long amount) {
            KLog.d("moneySpent() %d @ %s", amount +" ---- "+txId);
        }

        public void moneyReceived(String txId, long amount) {
            KLog.d("moneyReceived() %d @ %s", amount +" ---- "+txId);
        }

        public void unconfirmedMoneyReceived(String txId, long amount) {
            KLog.d("unconfirmedMoneyReceived() %d @ %s", amount +" ---- "+txId);
        }
        ...
        public void newBlock(long height) {
            ...
            KLog.d("MyWalletListener.newBlock() height= "+height);
            Wallet wallet = getWallet();
            if (wallet == null) throw new IllegalStateException("No wallet!");
            // don't flood with an update for every block ...
            if (lastBlockTime < System.currentTimeMillis() - 2000) {
                KLog.d("newBlock() @ %d with observer %s", height+"---"+observer);
    ...
    private final class ServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            KLog.d("Handling %s", msg.arg2);
            if (errorState) {
                KLog.i("In error state.");
            ...
            switch (msg.arg2) {
                case START_SERVICE: {
                    ...
                    if (cmd.equals(REQUEST_CMD_LOAD)) {
                        ...
                        KLog.d("STORE wallet: %s", myWallet.getName());
                        boolean rc = myWallet.store();
                        KLog.d("wallet stored: %s with rc=%b", myWallet.getName()+"----"+rc);
                        if (!rc) {
                            KLog.w("Wallet store failed: %s", myWallet.getErrorString());
                        }
                        if (observer != null) observer.onWalletStored(rc);
                    } else if (cmd.equals(REQUEST_CMD_TX)) {
                        Wallet myWallet = getWallet();
                        KLog.d("CREATE TX for wallet: %s", myWallet.getName());
                        ...
                        KLog.d("transaction status %s", status);
                        if (status != PendingTransaction.Status.Status_Ok) {
                            KLog.w("Create Transaction failed: %s", pendingTransaction.getErrorString());
                        }
                        if (observer != null) {
                        ...
                        Wallet myWallet = getWallet();
                        KLog.d("SWEEP TX for wallet: %s", myWallet.getName());
                        myWallet.disposePendingTransaction(); // remove any old pending tx
                        String txTag = extras.getString(REQUEST_CMD_TX_TAG);
                        ...
                        Wallet myWallet = getWallet();
                        KLog.d("SEND TX for wallet: %s", myWallet.getName());
                        ...
                        if (pendingTransaction.getStatus() != PendingTransaction.Status.Status_Ok) {
                            KLog.e("PendingTransaction is %s", pendingTransaction.getStatus());
                        ...
                        Wallet myWallet = getWallet();
                        KLog.d("SET NOTE for wallet: %s", myWallet.getName());
                        ...
                        if ((txId != null) && (notes != null)) {
                            ...
                            if (!success) {
                                KLog.e(myWallet.getErrorString());
                            }
    ...
    public void onDestroy() {
        ...
        if (this.listener != null) {
            KLog.w("onDestroy() with active listener");
    ...
    private Wallet loadWallet(String walletName, String walletPassword) {
        ...
        if (wallet != null) {
            KLog.d("Using daemon %s", WalletManager.getInstance().getDaemonAddress());
            showProgress(55);
            wallet.init(0);
            showProgress(90);
        }
