package org.blackey.utils.strategy.impl;
...
import org.blackey.entity.Stroke;
import org.blackey.utils.StrokeUtils;
...
public class StrokeSortStrategy implements ISortStrategy {
     ...
     */
    @Override
    public int getSideBarSortSectionFirstShowPosition(List<CountryOrRegion> list, String sideBarSortShowItem) {
        try {
            //提取字符串中的数字
            int strokeCount = Integer.parseInt(numberIntercept(sideBarSortShowItem));
            //然后查询出 这个数字首次出现的时候
            return getFirstOrLastPositionForSection(list, strokeCount, true);
        } catch (Exception e) {
