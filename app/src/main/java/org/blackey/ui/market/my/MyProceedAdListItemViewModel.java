...
public class MyProceedAdListItemViewModel extends MultiItemViewModel<MyAdsListViewModel> {
    ...
    public MyProceedAdListItemViewModel(@NonNull MyAdsListViewModel viewModel) {
        super(viewModel);
    }

    public MyProceedAdListItemViewModel(@NonNull MyAdsListViewModel viewModel, Advertise entity) {
        super(viewModel);
        multiType = R.layout.item_market_my_proceed_ad_list;
    ...
    //条目的点击事件
    public BindingCommand revocationClickCommand = new BindingCommand(new BindingConsumer<View>() {
        @Override
        public void call(View view) {
            //这里可以通过一个标识,做出判断，已达到跳入不同界面的逻辑
            showNormalDialog();
    ...
    private void showNormalDialog(){
        ...
        normalDialog.setTitle(viewModel.getApplication().getString(R.string.revocation_confirmation));
        normalDialog.setMessage(viewModel.getApplication().getString(R.string.hint_revocation_ad));
        ...
        RetrofitClient.getInstance().create(AdvertiseApiService.class)
                ...
                .subscribe(new Consumer<BaseResponse<Boolean>>() {
                    ...
                    public void accept(BaseResponse<Boolean> response) throws Exception {
                        //请求成功
                        if(response.isOk()){
                            viewModel.observableList.remove(entity.get());
                        }
