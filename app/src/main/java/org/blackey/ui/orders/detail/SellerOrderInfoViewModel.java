...
public class SellerOrderInfoViewModel extends ToolbarViewModel {
    ...
    public ObservableField<String> textSellerMind = new ObservableField<>();
    ...
    public SellerOrderInfoViewModel(@NonNull Application application) {
        super(application);
    }
    ...
    private void initData() {
        ...
        contactNumber.set("+"+entity.get().getBuyerNationCode() + entity.get().getBuyerMobile());


        String totalMoneyStr = totalMoneyText.get()+" " +entity.get().getCurrencyName();

        textSellerMind.set(getApplication().getString(R.string.text_seller_mind,totalMoneyStr));

        if(entity.get().getOrderStatus() == Order.ORDER_STATUS_UNPAID){
            rightButtonText.set(getApplication().getString(R.string.seller_order_status_unpaid));

        }else if(entity.get().getOrderStatus() == Order.ORDER_STATUS_PREPAID){
            rightButtonText.set(getApplication().getString(R.string.seller_status_prepaid));
                    ...
                    public void accept(BaseResponse<List<UserPaymentMode>> response) throws Exception {
                        ...
                        if(response.isOk()){
                            for(UserPaymentMode paymentMode : response.getBody()){
                                AccountViewPagerItemViewModel itemViewModel = new AccountViewPagerItemViewModel(SellerOrderInfoViewModel.this, paymentMode);
    ...
    public BindingCommand confirmationClick = new BindingCommand(new BindingConsumer<View>() {
        @Override
        public void call(View view) {
            if(entity.get().getOrderStatus() == Order.ORDER_STATUS_PREPAID){
                confirmReceipt();
            }
        }
    });
